...
- (void)setup {
    ...
    if(!_arrowCircleView) {
        _arrowCircleView = [[ZSDRefreshCircleView alloc]initWithFrame:CGRectMake(self.bounds.size.width/2 - 60, 19, kArrowSize.width, kArrowSize.height)];
        _arrowCircleView.image = [UIImage imageNamed:@"ico_bottom_arrow"];
        [self addSubview:_arrowCircleView];
    }
    
    if(!_textLabel) {
        _textLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_arrowCircleView.frame) + 13.0f, CGRectGetMinY(_arrowCircleView.frame), 120, kArrowSize.height)];
...
- (void)settingLabelText {
    ...
    NSString *down    = [bundle localizedStringForKey:@"drop-down" value:nil table:@"localizable"];
    NSString *refresh = [bundle localizedStringForKey:@"release_refresh" value:nil table:@"localizable"];
    NSString *load    = [bundle localizedStringForKey:@"being_loaded" value:nil table:@"localizable"];
    // 设置文字
    switch (self.state) {
        case ZSDRefreshStateNormal:
        case ZSDRefreshStatePulling:
            self.textLabel.text = down;
            break;
        case ZSDRefreshStateWillRefreshing:
            self.textLabel.text = refresh;
            break;
        case ZSDRefreshStateRefreshing:
            self.textLabel.text = load;
            break;
        default:
            break;
    }
