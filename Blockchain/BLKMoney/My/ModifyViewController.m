    ...
    if ([userDefaults objectForKey:USER_TOKEN]) {
        ...
        [networkRequest patchUrl:url header:@"Authorization" value:token parameters:parameters success:^(id responseObject) {
            ...
            dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                    [[NSUserDefaults standardUserDefaults] setObject:new forKey:USER_PASSWORD];
...
- (void)createView {
    ...
    CGFloat t_x = oldLabel.right+10;
    CGFloat t_w = SCREEN_W-t_x-10;
    NSString *password1 = [bundle localizedStringForKey:@"original_password1" value:nil table:@"localizable"];
    oldTextField = [[UITextField alloc]initWithFrame:CGRectMake(t_x, 0, t_w, l_h)];
    oldTextField.delegate = self;
    oldTextField.placeholder = password1;
    ...
    newTextField.placeholder = new_password1;
    ...
    NSString *confirm = [bundle localizedStringForKey:@"confirm_password1" value:nil table:@"localizable"];
    sureTextField = [[UITextField alloc]initWithFrame:CGRectMake(t_x, newLabel.bottom, t_w, l_h)];
    sureTextField.delegate = self;
    sureTextField.placeholder = confirm;
    ...
    NSString *confirm1 = [bundle localizedStringForKey:@"confirm" value:nil table:@"localizable"];
    sureButton = [UIButton buttonWithType:UIButtonTypeSystem];
    sureButton.frame = CGRectMake(l_x, textView.bottom+l_x, SCREEN_W-l_x*2, b_h);
    sureButton.layer.cornerRadius = 5;
    sureButton.layer.masksToBounds = YES;
    sureButton.backgroundColor = [MAINCOLOR colorWithAlphaComponent:0.3];
    [sureButton setTitle:confirm1 forState:UIControlStateNormal];
...
- (BOOL)isValidateTextField {
    ...
    NSString *old1     = [bundle localizedStringForKey:@"original_password1" value:nil table:@"localizable"];
    NSString *new1  = [bundle localizedStringForKey:@"new_password1" value:nil table:@"localizable"];
    ...
    if ([old isEqualToString:@""]) {
        hudText = [[ZZPhotoHud alloc] init];
        [hudText showActiveHud:old1];
        return NO;
    } else if ([new isEqualToString:@""]) {
        hudText = [[ZZPhotoHud alloc] init];
        [hudText showActiveHud:new1];
        return NO;
