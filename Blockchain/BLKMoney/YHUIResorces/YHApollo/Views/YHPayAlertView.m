...
@interface YHPayAlertView () <UITextFieldDelegate>
...
- (void)show {
    [self jk_showInWindowWithMode:JKCustomAnimationModeAlert inView:nil bgAlpha:0.4 needEffectView:NO];
    /*
...
- (UIView *)paymentAlert {
    if (_paymentAlert == nil) {
        _paymentAlert = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.width, self.height)];
...
- (UILabel *)detailLabel {
    if (_detailLabel == nil) {
        _detailLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, kTitleHeight+15+self.inputView.frame.size.height + 15, 30, 20)];
//        _detailLabel.textAlignment = NSTextAlignmentCenter;
        _detailLabel.textColor = [UIColor darkGrayColor];
        _detailLabel.font = [UIFont systemFontOfSize:14];
        
        _detailLabel.text = @"提示：";
    }
    return _detailLabel;
}
...
- (UILabel *)amountLabel {
    if (_amountLabel == nil) {
        _amountLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.detailLabel.frame), self.detailLabel.frame.origin.y, kPAYMENT_WIDTH-30, 40)];
        ...
        _amountLabel.text = @"45天内关闭Apollo收取5%服务费\n45天后关闭Apollo收取1%服务费";
    }
    return _amountLabel;
}
...
- (UIView *)inputView {
    if (_inputView == nil) {
        _inputView = [[UIView alloc]initWithFrame:CGRectMake(15, kTitleHeight+15, kPAYMENT_WIDTH-30, (kPAYMENT_WIDTH-30)/6)];
